...
public class Dnn {
     ...
     * href="https://pjreddie.com/darknet/"&gt;Darknet&lt;/a&gt; model files.
     *
     * @param bufferCfg A buffer contains a content of .cfg file with text description of the
     ...
     *
     * @param model path to the .pb file with binary protobuf description of the network
     *     architecture
     ...
     *
     * @param model path to the .pb file with binary protobuf description of the network
     *     architecture Resulting Net object is built by text graph using weights from a binary one
     *     that let us make it more flexible.
     * @return Net object.
     */
    ...
    public static Net readNetFromTensorflow(MatOfByte bufferModel, MatOfByte bufferConfig) {
        ...
        return new Net(
                readNetFromTensorflow_2(bufferModel_mat.nativeObj, bufferConfig_mat.nativeObj));
    }
     ...
     * @param evaluate specifies testing phase of network. If true, it's similar to evaluate()
     *     method in Torch.
     * @return Net object.
     *     <p><b>Note:</b> Ascii mode of Torch serializer is more preferable, because binary mode
     *     extensively use {@code long} type of C language, which has various bit-length on
     *     different systems.
     *     <p>The loading file must contain serialized &lt;a
     *     href="https://github.com/torch/nn/blob/master/doc/module.md"&gt;nn.Module&lt;/a&gt;
     *     object with importing network. Try to eliminate a custom objects from serialazing data to
     ...
     *     nn.SpatialMaxPooling, nn.SpatialAveragePooling - nn.ReLU, nn.TanH, nn.Sigmoid -
     *     nn.Reshape - nn.SoftMax, nn.LogSoftMax
     *     <p>Also some equivalents of these classes from cunn, cudnn, and fbcunn may be
     ...
     *     http://caffe.berkeleyvision.org/) * {@code *.pb} (TensorFlow,
     *     https://www.tensorflow.org/) * {@code *.t7} | {@code *.net} (Torch, http://torch.ch/) *
     *     {@code *.weights} (Darknet, https://pjreddie.com/darknet/) * {@code *.bin} (DLDT,
     ...
     *     {@code *.weights} (Darknet, https://pjreddie.com/darknet/) * {@code *.bin} (DLDT,
     *     https://software.intel.com/openvino-toolkit) * {@code *.onnx} (ONNX, https://onnx.ai/)
     *     file with the following extensions: * {@code *.prototxt} (Caffe,
    ...
    public static Net readNetFromModelOptimizer(
        ...
        return new Net(
                readNetFromModelOptimizer_1(
                        bufferModelConfig_mat.nativeObj, bufferWeights_mat.nativeObj));
    }
    ...
    /**
     * Creates 4-dimensional blob from image. Optionally resizes and crops {@code image} from
     * center, subtract {@code mean} values, scales values by {@code scalefactor}, swap Blue and Red
     * channels.
     *
     * @param image input image (with 1-, 3- or 4-channels).
     * @param size spatial size for output image
     * @param mean scalar with mean values which are subtracted from channels. Values are intended
     *     to be in (mean-R, mean-G, mean-B) order if {@code image} has BGR ordering and {@code
     *     swapRB} is true.
     ...
     *     dimension in {@code size} and another one is equal or larger. Then, crop from the center
     *     is performed. If {@code crop} is false, direct resize without cropping and preserving
     ...
     *     equal to corresponding dimension in {@code size} and another one is equal or larger.
     *     Then, crop from the center is performed. If {@code crop} is false, direct resize without
     ...
     *     corresponding dimension in {@code size} and another one is equal or larger. Then, crop
     *     from the center is performed. If {@code crop} is false, direct resize without cropping
     ...
     * from center, subtract {@code mean} values, scales values by {@code scalefactor}, swap Blue
     * and Red channels.
     *
    ...
    public static Mat blobFromImages(List<Mat> images, double scalefactor, Size size) {
        Mat images_mat = Converters.vector_Mat_to_Mat(images);
        return new Mat(
                blobFromImages_4(images_mat.nativeObj, scalefactor, size.width, size.height));
    }
    ...
    /**
     * Parse a 4D blob and output the images it contains as 2D arrays through a simpler data
     * structure (std::vector&lt;cv::Mat&gt;).
     *
     * @param blob_ 4 dimensional array (images, channels, height, width) in floating point
     *     precision (CV_32F) from which you would like to extract the images.
     * @param images_ array of 2D Mat containing the images extracted from the blob in floating
     *     point precision (CV_32F). They are non normalized neither mean added. The number of
     *     returned images equals the first dimension of the blob (batch size). Every image has a
     *     number of channels equals to the second dimension of the blob (depth).
     */
     ...
     * @param layersTypes Set of layers types which parameters will be converted. By default,
     *     converts only Convolutional and Fully-Connected layers' weights.
     *     <p><b>Note:</b> Shrinked model has no origin float32 weights so it can't be used in
     *     origin Caffe framework anymore. However the structure of data is taken from NVidia's
     *     Caffe fork: https://github.com/NVIDIA/caffe. So the resulting model may be used there.
     */
     ...
     *
     * <p>Default value is controlled through {@code OPENCV_DNN_BACKEND_INFERENCE_ENGINE_TYPE}
     ...
     *
     * <p>Specify OpenVINO plugin: CPU or ARM.
     *
     * @return automatically generated
     */
    ...
    // C++:  Net cv::dnn::readNet(String framework, vector_uchar bufferModel, vector_uchar
    // bufferConfig = std::vector<uchar>())
    private static native long readNet_3(
