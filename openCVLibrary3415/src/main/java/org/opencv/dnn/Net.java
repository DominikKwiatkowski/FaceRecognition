...
public class Net {
     ...
     * @param bin Binary file with trained weights. Networks imported from Intel's Model Optimizer
     *     are launched in Intel's Inference Engine backend.
     ...
     *     template &lt;DFN&gt;layer_name&lt;/DFN&gt; is string name of the added layer. If this
     *     part is empty then the network input pseudo layer will be used; - the second optional
     *     part of the template &lt;DFN&gt;input_number&lt;/DFN&gt; is either number of the layer
     *     input, either label one. If this part is omitted then the first layer input will be used.
     ...
     *
     * <p>Each net always has special own the network input pseudo layer with id=0. This layer
     * stores the user blobs only and don't make any computations. In fact, this layer provides the
     * only way to pass user data into the network. As any other layer, this layer can label its
     * outputs and this function provides an easy way to do this.
     *
     ...
     * @param outputName name for layer which output is needed to get If {@code outputName} is
     ...
     *     <p>Schedule layers that support Halide backend. Then compile them for specific target.
     *     For layers that not represented in scheduling file or if no manual scheduling used at
     *     all, automatic scheduling will be applied.
     */
    ...
    //
    // C++:  void cv::dnn::Net::setInput(Mat blob, String name = "", double scalefactor = 1.0,
    // Scalar mean = Scalar())
    //
     ...
     * @param mean An optional mean subtraction values. SEE: connect(String, String) to know format
     *     of the descriptor.
     ...
     * @param blob A new blob. Should have CV_32F or CV_8U depth.
     * @param name A name of input layer.
     * @param scalefactor An optional normalization scale. SEE: connect(String, String) to know
     ...
     * Sets the new input value for the network
     *
     * @param blob A new blob. Should have CV_32F or CV_8U depth. SEE: connect(String, String) to
     *     know format of the descriptor.
     *     <p>If scale or mean values are specified, a final input blob is computed as:
     *     \(input(n,c,h,w) = scalefactor \times (blob(n,c,h,w) - mean_c)\)
     */
     ...
     * @param blob the new value. SEE: Layer::blobs <b>Note:</b> If shape of the new blob differs
     *     from the previous shape, then the following forward pass may fail.
     */
    ...
    //
    // C++:  void cv::dnn::Net::getLayersShapes(vector_MatShape netInputShapes, vector_int&
    // layersIds, vector_vector_MatShape& inLayersShapes, vector_vector_MatShape& outLayersShapes)
    //
    ...
    //
    // C++:  void cv::dnn::Net::getMemoryConsumption(MatShape netInputShape, size_t& weights,
    // size_t& blobs)
    //
    ...
    public void getMemoryConsumption(
        ...
        getMemoryConsumption_2(
                nativeObj, layerId, netInputShape_mat.nativeObj, weights_out, blobs_out);
        if (weights != null) weights[0] = (long) weights_out[0];
        if (blobs != null) blobs[0] = (long) blobs_out[0];
    }
     ...
     *
     * <p>Indexes in returned vector correspond to layers ids. Some layers can be fused with others,
     * in this case zero ticks count will be return for that skipped layers. Supported by
    ...
    // C++:  int64 cv::dnn::Net::getFLOPS(int layerId, vector_MatShape netInputShapes)
    private static native long getFLOPS_2(
            long nativeObj, int layerId, List<MatOfInt> netInputShapes);

    // C++:  int64 cv::dnn::Net::getFLOPS(int layerId, MatShape netInputShape)
    private static native long getFLOPS_3(
