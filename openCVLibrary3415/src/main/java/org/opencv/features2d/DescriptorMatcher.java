...
public class DescriptorMatcher extends Algorithm {
     ...
     *
     * <p>If the collection is not empty, the new descriptors are added to existing train
     * descriptors.
     *
     ...
     *     argument. In the second variant of the method, train descriptors collection that was set
     *     by DescriptorMatcher::add is used. Optional mask (or masks) can be passed to specify
     *     which query and training descriptors can be matched. Namely, queryDescriptors[i] can be
     *     matched with trainDescriptors[j] only if mask.at&lt;uchar&gt;(i,j) is non-zero.
     */
     ...
     * @param compactResult Parameter used when the mask (or masks) is not empty. If compactResult
     *     is false, the matches vector has the same size as queryDescriptors rows. If compactResult
     *     is true, the matches vector does not contain matches for fully masked-out query
     *     descriptors.
    ...
    // vector_vector_DMatch& matches, float maxDistance, Mat mask = Mat(), bool compactResult =
    // false)
    //
     ...
     *     is true, the matches vector does not contain matches for fully masked-out query
     *     descriptors.
     * @param maxDistance Threshold for the distance between matched descriptors. Distance means
     *     here metric distance (e.g. Hamming distance), not the distance between coordinates (which
     *     is measured in Pixels)!
     * @param mask Mask specifying permissible matches between an input query and train matrices of
     *     descriptors.
     *     <p>For each query descriptor, the methods find such training descriptors that the
     *     distance between the query descriptor and the training descriptor is equal or smaller
     *     than maxDistance. Found matches are returned in the distance increasing order.
     */
     ...
     * @param maxDistance Threshold for the distance between matched descriptors. Distance means
     *     here metric distance (e.g. Hamming distance), not the distance between coordinates (which
     *     is measured in Pixels)! descriptors.
     *     <p>For each query descriptor, the methods find such training descriptors that the
     *     distance between the query descriptor and the training descriptor is equal or smaller
     *     than maxDistance. Found matches are returned in the distance increasing order.
     */
     ...
     * @param matches Matches. If a query descriptor is masked out in mask , no match is added for
     *     this descriptor. So, matches size may be smaller than the query descriptors count.
     * @param masks Set of masks. Each masks[i] specifies permissible matches between the input
     *     query descriptors and stored train descriptors from the i-th image
    ...
    //
    // C++:  void cv::DescriptorMatcher::knnMatch(Mat queryDescriptors, vector_vector_DMatch&
    // matches, int k, vector_Mat masks = vector_Mat(), bool compactResult = false)
    //
     ...
     * @param maxDistance Threshold for the distance between matched descriptors. Distance means
     *     here metric distance (e.g. Hamming distance), not the distance between coordinates (which
     *     is measured in Pixels)! descriptors and stored train descriptors from the i-th image
     *     trainDescCollection[i]. false, the matches vector has the same size as queryDescriptors
     *     rows. If compactResult is true, the matches vector does not contain matches for fully
     *     masked-out query descriptors.
     */
     ...
     * Clones the matcher.
     *
     * @param emptyTrainData If emptyTrainData is false, the method creates a deep copy of the
     *     object, that is, copies both parameters and train data. If emptyTrainData is true, the
     *     method creates an object copy with the current parameters but with empty train data.
     * @return automatically generated
     */
