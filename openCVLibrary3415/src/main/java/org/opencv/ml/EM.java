...
public class EM extends StatModel {
     ...
     *
     * <p>Returns matrix with the number of rows equal to the number of mixtures and number of
     * columns equal to the space dimensionality.
     *
     * @return automatically generated
     */
    public Mat getMeans() {
     ...
     * Returns posterior probabilities for the provided samples
     *
     * @param samples The input samples, floating-point matrix posterior probabilities for each
     ...
     *     value for the sample. First element is an index of the most probable mixture component
     ...
     *
     * <p>This variation starts with Expectation step. Initial values of the model parameters will
     * be estimated by the k-means algorithm.
     *
     * <p>Unlike many of the ML models, %EM is an unsupervised learning algorithm and it does not
     * take responses (class labels or function values) as input. Instead, it computes the *Maximum
     ...
     *
     * @param samples Samples from which the Gaussian mixture model will be estimated. It should be
     *     a one-channel matrix, each row of which is a sample. If the matrix does not have CV_64F
     *     type it will be converted to the inner matrix of such type for the further computing.
     * @param logLikelihoods The optional output matrix that contains a likelihood logarithm value
     *     for each sample. It has \(nsamples \times 1\) size and CV_64FC1 type.
     ...
     * @param probs The optional output matrix that contains posterior probabilities of each
     *     Gaussian mixture component given the each sample. It has \(nsamples \times nclusters\)
    ...
    //
    // C++:  bool cv::ml::EM::trainE(Mat samples, Mat means0, Mat covs0 = Mat(), Mat weights0 =
    // Mat(), Mat& logLikelihoods = Mat(), Mat& labels = Mat(), Mat& probs = Mat())
    //
     ...
     *     covariance matrices is a one-channel matrix of \(dims \times dims\) size. If the matrices
     *     do not have CV_64F type they will be converted to the inner matrices of such type for the
     *     further computing. floating-point matrix with \(1 \times nclusters\) or \(nclusters
     *     \times 1\) size. each sample. It has \(nsamples \times 1\) size and CV_64FC1 type.
     *     \(\texttt{labels}_i=\texttt{arg max}_k(p_{i,k}), i=1..N\) (indices of the most probable
     *     mixture component for each sample). It has \(nsamples \times 1\) size and CV_32SC1 type.
     *     mixture component given the each sample. It has \(nsamples \times nclusters\) size and
     *     CV_64FC1 type.
    ...
    //
    // C++:  bool cv::ml::EM::trainM(Mat samples, Mat probs0, Mat& logLikelihoods = Mat(), Mat&
     ...
     *     probable mixture component for each sample). It has \(nsamples \times 1\) size and
     *     CV_32SC1 type. mixture component given the each sample. It has \(nsamples \times
     *     nclusters\) size and CV_64FC1 type.
     * @return automatically generated
     */
