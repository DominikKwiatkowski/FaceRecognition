...
public class HOGDescriptor {
     ...
     *
     * <p>aqual to HOGDescriptor(Size(64,128), Size(16,16), Size(8,8), Size(8,8), 9, 1 )
     */
    ...
    //
    // C++:   cv::HOGDescriptor::HOGDescriptor(Size _winSize, Size _blockSize, Size _blockStride,
    // Size _cellSize, int _nbins, int _derivAperture = 1, double _winSigma = -1, int
    // _histogramNormType = HOGDescriptor::L2Hys, double _L2HysThreshold = 0.2, bool
    // _gammaCorrection = false, int _nlevels = HOGDescriptor::DEFAULT_NLEVELS, bool _signedGradient
    // = false)
    //
    ...
    /**
     * @param filename the file name containing HOGDescriptor properties and coefficients of the
     *     trained classifier
     */
     ...
     *
     * @param img Matrix of the type CV_8U containing an image where HOG features will be
     *     calculated.
    ...
    // weights, double hitThreshold = 0, Size winStride = Size(), Size padding = Size(),
     ...
     *
     * @param img Matrix of the type CV_8U or CV_8UC3 containing an image where objects are
     *     detected.
     ...
     *     coefficient). But if the free coefficient is omitted (which is allowed), you can specify
     *     it manually here.
     * @param winStride Window stride. It must be a multiple of block stride.
     * @param padding Padding
     ...
     */
    public void detect(
            Mat img, MatOfPoint foundLocations, MatOfDouble weights, double hitThreshold) {
        Mat foundLocations_mat = foundLocations;
        Mat weights_mat = weights;
        detect_3(
                nativeObj,
     ...
     * @param foundLocations Vector of point where each point contains left-top corner point of
     *     detected object boundaries.
     * @param weights Vector that will contain confidence values for each detected object. Usually
     *     it is 0 and should be specified in the detector coefficients (as the last free
     *     coefficient). But if the free coefficient is omitted (which is allowed), you can specify
     *     it manually here.
     */
    ...
    //
    // C++:  void cv::HOGDescriptor::computeGradient(Mat img, Mat& grad, Mat& angleOfs, Size
    ...
    private static native long HOGDescriptor_8(
            double _winSize_width,
            double _winSize_height,
            double _blockSize_width,
            double _blockSize_height,
