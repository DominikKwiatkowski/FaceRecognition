...
public class Photo {
     ...
     *     <p>The function reconstructs the selected image area from the pixel near the area
     *     boundary. The function may be used to remove dust and scratches from a scanned photo, or
     *     to remove undesirable objects from still images or video. See
    ...
    //
    // C++:  void cv::fastNlMeansDenoising(Mat src, Mat& dst, float h = 3, int templateWindowSize =
     ...
     * @param dst Output image with the same size and type as src .
     * @param templateWindowSize Size in pixels of the template patch that is used to compute
    ...
    public static void fastNlMeansDenoising(
        ...
        fastNlMeansDenoising_0(
                src.nativeObj, dst.nativeObj, h, templateWindowSize, searchWindowSize);
    }
     ...
     * @param dst Output image with the same size and type as src .
     * @param templateWindowSize Size in pixels of the template patch that is used to compute
     *     weights. Should be odd. Recommended value 7 pixels given pixel. Should be odd. Affect
     *     performance linearly: greater searchWindowsSize - greater denoising time. Recommended
     *     value 21 pixels
     * @param h Parameter regulating filter strength. Big h value perfectly removes noise but also
     *     removes image details, smaller h value preserves details but also preserves some noise
     *     <p>This function expected to be applied to grayscale images. For colored images look at
     ...
     *
     * @param src Input 8-bit 1-channel, 2-channel, 3-channel or 4-channel image.
     * @param dst Output image with the same size and type as src . Should be odd. Recommended value
     *     7 pixels given pixel. Should be odd. Affect performance linearly: greater
     ...
     * @param dst Output image with the same size and type as src . Should be odd. Recommended value
     *     7 pixels given pixel. Should be odd. Affect performance linearly: greater
     *     searchWindowsSize - greater denoising time. Recommended value 21 pixels removes image
     ...
     */
    public static void fastNlMeansDenoising(
            Mat src, Mat dst, MatOfFloat h, int templateWindowSize) {
        Mat h_mat = h;
     ...
     *     for given pixel. Should be odd. Affect performance linearly: greater searchWindowsSize -
     *     greater denoising time. Recommended value 21 pixels
     * @param h Parameter regulating filter strength for luminance component. Bigger h value
     *     perfectly removes noise but also removes image details, smaller h value preserves details
     ...
     * @param h Parameter regulating filter strength for luminance component. Bigger h value
     *     perfectly removes noise but also removes image details, smaller h value preserves details
     *     but also preserves some noise will be enough to remove colored noise and do not distort
     *     colors
     *     <p>The function converts image to CIELAB colorspace and then separately denoise L and AB
     *     components with given h parameters using fastNlMeansDenoising function.
     */
     ...
     *     noise will be enough to remove colored noise and do not distort colors
     *     <p>The function converts image to CIELAB colorspace and then separately denoise L and AB
     *     components with given h parameters using fastNlMeansDenoising function.
     */
    ...
    /**
     * Modification of fastNlMeansDenoising function for images sequence where consecutive images
     * have been captured in small period of time. For example video. This version of the function
     * is for grayscale images or for manual manipulation with colorspaces. For more details see
     ...
     *     Should be odd. Images from imgToDenoiseIndex - temporalWindowSize / 2 to
     ...
     *     for given pixel. Should be odd. Affect performance linearly: greater searchWindowsSize -
     *     greater denoising time. Recommended value 21 pixels
     * @param h Parameter regulating filter strength. Bigger h value perfectly removes noise but
     *     also removes image details, smaller h value preserves details but also preserves some
     *     noise
     */
     ...
     *     Recommended value 7 pixels given pixel. Should be odd. Affect performance linearly:
     ...
     * @param dst Output image with the same size and type as srcImgs images. Should be odd.
     *     Recommended value 7 pixels given pixel. Should be odd. Affect performance linearly:
     *     greater searchWindowsSize - greater denoising time. Recommended value 21 pixels perfectly
    ...
    //
    // C++:  void cv::fastNlMeansDenoisingMulti(vector_Mat srcImgs, Mat& dst, int imgToDenoiseIndex,
    // int temporalWindowSize, vector_float h, int templateWindowSize = 7, int searchWindowSize =
    // 21, int normType = NORM_L2)
    //
     ...
     *
     * @param srcImgs Input 8-bit 3-channel images sequence. All images should have the same type
     *     and size.
     * @param imgToDenoiseIndex Target image to denoise index in srcImgs sequence
     * @param temporalWindowSize Number of surrounding images to use for target image denoising.
     *     Should be odd. Images from imgToDenoiseIndex - temporalWindowSize / 2 to
     ...
     * @param dst Output image with the same size and type as srcImgs images. Should be odd.
     *     Recommended value 7 pixels given pixel. Should be odd. Affect performance linearly:
     *     greater searchWindowsSize - greater denoising time. Recommended value 21 pixels removes
     ...
     * is, finding a function to minimize some functional). As the image denoising, in particular,
     * may be seen as the variational problem, primal-dual algorithm then can be used to perform
     ...
     *
     * <p>Although the thorough discussion and justification of the algorithm involved may be found
     * in CITE: ChambolleEtAl, it might make sense to skim over it here, following CITE: MA13 . To
     * begin with, we consider the 1-byte gray-level images as the functions from the rectangular
     * domain of pixels (it may be seen as set \(\left\{(x,y)\in\mathbb{N}\times\mathbb{N}\mid 1\leq
     * x\leq n,\;1\leq y\leq m\right\}\) for some \(m,\;n\in\mathbb{N}\)) into
     * \(\{0,1,\dots,255\}\). We shall denote the noised images as \(f_i\) and with this view, given
     * some image \(x\) of the same size, we may measure how bad it is by the formula
     *
     * <p>\(\left\|\left\|\nabla x\right\|\right\| +
     * \lambda\sum_i\left\|\left\|x-f_i\right\|\right\|\)
     *
     * <p>\(\|\|\cdot\|\|\) here denotes \(L_2\)-norm and as you see, the first addend states that
     * we want our image to be smooth (ideally, having zero gradient, thus being constant) and the
     * second states that we want our result to be close to the observations we've got. If we treat
     * \(x\) as a function, this is exactly the functional what we seek to minimize and here the
     * Primal-Dual algorithm comes into play.
     *
     * @param observations This array should contain one or more noised versions of the image that
     *     is to be restored.
     * @param result Here the denoised image will be stored. There is no need to do pre-allocation
     *     of storage space, as it will be automatically allocated, if necessary.
     ...
     *     as better, but it is hard to quantitatively refine this statement, so just use the
     ...
     *     is to be restored.
     * @param result Here the denoised image will be stored. There is no need to do pre-allocation
     *     of storage space, as it will be automatically allocated, if necessary. (blurred) images
     *     are treated more favorably than detailed (but maybe more noised) ones. Roughly speaking,
     *     as it becomes smaller, the result will be more blur but more sever outliers will be
     *     removed. better, but it is hard to quantitatively refine this statement, so just use the
     *     default and increase it if the results are poor.
     */
     ...
     * Creates simple linear mapper with gamma correction
     *
     * <p>equal to 2.2f is suitable for most displays. Generally gamma &gt; 1 brightens the image
     ...
     * Creates TonemapDrago object
     *
     * @param gamma gamma value for gamma correction. See createTonemap than 1 increase saturation
     *     and values less than 1 decrease it. results, default value is 0.85.
     * @return automatically generated
     */
    ...
    //
    // C++:  Ptr_TonemapReinhard cv::createTonemapReinhard(float gamma = 1.0f, float intensity =
    // 0.0f, float light_adapt = 1.0f, float color_adapt = 0.0f)
    //
     ...
     *     results.
     * @param light_adapt light adaptation in [0, 1] range. If 1 adaptation is based only on pixel
     *     value, if 0 it's global, otherwise it's a weighted mean of this two cases. if 0
    ...
    //
    // C++:  Ptr_TonemapMantiuk cv::createTonemapMantiuk(float gamma = 1.0f, float scale = 0.7f,
     ...
     * Creates TonemapMantiuk object
     *
     * @param gamma gamma value for gamma correction. See createTonemap dynamic range. Values from
     *     0.6 to 0.9 produce best results.
     * @return automatically generated
     */
    ...
    //
    // C++:  Ptr_AlignMTB cv::createAlignMTB(int max_bits = 6, int exclude_range = 4, bool cut =
    // true)
    //
    ...
    public static CalibrateDebevec createCalibrateDebevec(
            int samples, float lambda, boolean random) {
     ...
     * @param threshold target difference between results of two successive steps of the
     *     minimization.
     * @return automatically generated
     */
     ...
     * stylized black-and-white photograph rendering, and in many single channel image processing
     * applications CITE: CL12 .
     *
     * @param src Input 8-bit 3-channel image.
     ...
     * local changes, ones that are restricted to a region manually selected (ROI), in a seamless
     * and effortless manner. The extent of the changes ranges from slight distortions to complete
    ...
    public static void seamlessClone(Mat src, Mat dst, Mat mask, Point p, Mat blend, int flags) {
        seamlessClone_0(
                src.nativeObj, dst.nativeObj, mask.nativeObj, p.x, p.y, blend.nativeObj, flags);
    }
     ...
     * and then integrating back with a Poisson solver, modifies locally the apparent illumination
     * of an image.
     *
     * @param src Input 8-bit 3-channel image.
     * @param mask Input 8-bit 1 or 3-channel image.
    ...
    /**
     * By retaining only the gradients at edge locations, before integrating with the Poisson
     * solver, one washes out the texture of the selected region, giving its contents a flat aspect.
    ...
    //
    // C++:  void cv::edgePreservingFilter(Mat src, Mat& dst, int flags = 1, float sigma_s = 60,
    // float sigma_r = 0.4f)
    //

    /**
     * Filtering is the fundamental operation in image and video processing. Edge-preserving
     * smoothing filters are used in many different applications CITE: EM11 .
     *
     * @param src Input 8-bit 3-channel image.
    ...
    public static void pencilSketch(
        ...
        pencilSketch_0(
                src.nativeObj, dst1.nativeObj, dst2.nativeObj, sigma_s, sigma_r, shade_factor);
    }
     ...
     * photorealism. Edge-aware filters are ideal for stylization, as they can abstract regions of
     * low contrast while preserving, or enhancing, high-contrast features.
     *
     * @param src Input 8-bit 3-channel image.
     * @param dst Output image with the same size and type as src.
     * @param sigma_s %Range between 0 to 200.
    ...
    private static native void fastNlMeansDenoisingColored_4(
    ...
    private static native void denoise_TVL1_2(
            long observations_mat_nativeObj, long result_nativeObj);
    ...
    private static native void detailEnhance_1(
            long src_nativeObj, long dst_nativeObj, float sigma_s);
